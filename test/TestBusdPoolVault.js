...
describe('Test BellaFlexsaving BUSD Vault', vaultTestSuite('BUSD'))

function vaultTestSuite(strategyTokenSymbol) {
    return () => {
        const poolTokenSymbol = 'busd'
        const poolParam = curvePoolConstant[poolTokenSymbol].param
        const curveBusdPoolAddress = poolParam.curvePoolAddress
        ...
        describe('Test Vault deposit-1', () => {
            const day = 24 * 60 * 60
            const curveBusdPoolInstance = new web3.eth.Contract(curveBusdPool.abiArray, curveBusdPoolAddress)
            ...
            beforeEach(async (done) => {
                ...
                {
                    // prepare strategy Token balance(10000 BUSD) for governance
                    ...
                    // say 5 days passed and some exchange happened in curve BusdPool
                    await timeMachine.advanceTimeAndBlock(5 * day)
                    // swap 4 * 5 million volume during 5 days
                    await AccountUtils.giveERC20Token('USDT', governance, BNUtils.mul10pow(new BigNumber('2'), 6 + tokenAddress.USDT.decimals))
                    await AccountUtils.giveERC20Token('USDC', governance, BNUtils.mul10pow(new BigNumber('2'), 6 + tokenAddress.USDC.decimals))
                    await AccountUtils.giveERC20Token('DAI', governance, BNUtils.mul10pow(new BigNumber('2'), 6 + tokenAddress.DAI.decimals))
                    await AccountUtils.giveERC20Token('BUSD', governance, BNUtils.mul10pow(new BigNumber('2'), 6 + tokenAddress.BUSD.decimals))
                    await AccountUtils.doApprove('USDT', governance, curveBusdPoolAddress, BNUtils.mul10pow(new BigNumber('1').muln(5), 7 + tokenAddress.USDT.decimals))
                    await AccountUtils.doApprove('USDC', governance, curveBusdPoolAddress, BNUtils.mul10pow(new BigNumber('1').muln(5), 7 + tokenAddress.USDC.decimals))
                    await AccountUtils.doApprove('DAI', governance, curveBusdPoolAddress, BNUtils.mul10pow(new BigNumber('1').muln(5), 7 + tokenAddress.DAI.decimals))
                    await AccountUtils.doApprove('BUSD', governance, curveBusdPoolAddress, BNUtils.mul10pow(new BigNumber('1').muln(5), 7 + tokenAddress.BUSD.decimals))
                    for (let i = 0; i < 5; i++) {
                        await curveBusdPoolInstance.methods.exchange_underlying(poolParam.UNDERLYING_POOL_TOKEN.findIndex('DAI'), poolParam.UNDERLYING_POOL_TOKEN.findIndex('USDC'), BNUtils.mul10pow(new BigNumber('1'), 6 + tokenAddress.DAI.decimals).toString(), 0).send({ from: governance, gas: 5000000 })
                        await curveBusdPoolInstance.methods.exchange_underlying(poolParam.UNDERLYING_POOL_TOKEN.findIndex('USDC'), poolParam.UNDERLYING_POOL_TOKEN.findIndex('USDT'), BNUtils.mul10pow(new BigNumber('1'), 6 + tokenAddress.USDC.decimals).toString(), 0).send({ from: governance, gas: 5000000 })
                        await curveBusdPoolInstance.methods.exchange_underlying(poolParam.UNDERLYING_POOL_TOKEN.findIndex('USDT'), poolParam.UNDERLYING_POOL_TOKEN.findIndex('BUSD'), BNUtils.mul10pow(new BigNumber('1'), 6 + tokenAddress.USDT.decimals).toString(), 0).send({ from: governance, gas: 5000000 })
                        await curveBusdPoolInstance.methods.exchange_underlying(poolParam.UNDERLYING_POOL_TOKEN.findIndex('BUSD'), poolParam.UNDERLYING_POOL_TOKEN.findIndex('DAI'), BNUtils.mul10pow(new BigNumber('1'), 6 + tokenAddress.BUSD.decimals).toString(), 0).send({ from: governance, gas: 5000000 })
                    }
                }
                // prepare user Token balance(10000 BUSD) for testUser
            ...
            // And bBUSD to BUSD exchange rate equals 1.0100 *which is, let someone deposit first and then, reinvest some amount to strategy pool and do some exchange in curve pool
            // When userA deposits 10000 BUSD
            // Then the bBUSD balance of userA’s wallet address(0x) equals 9900.9901
        ...
        describe('Test Vault deposit-2', () => {
            ...
            // And the bVault is not paused
            // When contractA deposits any amount of BUSD
        ...
        describe('Test Vault earn-1', () => {
            ...
            beforeEach(async (done) => {
                // prepare user Token balance(250000 BUSD) for testUser and deposit to vault
                let userTokenAmountToDeposit = BNUtils.mul10pow(new BigNumber('250000'), tokenAddress[strategyTokenSymbol].decimals)
                await AccountUtils.giveERC20Token(strategyTokenSymbol, testUser, userTokenAmountToDeposit)
                await AccountUtils.doApprove(strategyTokenSymbol, testUser, vaultAddress, userTokenAmountToDeposit)
                await send(vault, 'deposit', [userTokenAmountToDeposit.toString()], { from: testUser })
                // prepare strategy Token balance(750000 BUSD) for governance and transfer to strategy
            ...
            // Happy Path #1
            // Given BUSD vault balance equals 250,000
            // And BUSD vault Buffer equals 10%
            // And BUSD vault total BUSD deposit equals 1,000,000
            // When userA (anyone) call Earn at BUSD vault
            // Then the amount of BUSD deployed from BUSD vault to BUSD strategy equals 150,000
            // Then the remaining amount of BUSD in BUSD vault equals 100,000
        ...
        describe('Test Vault earn-2', () => {
            ...
            // Unhappy Path #1:
            // Given BUSD vault balance equals 70,000
            // And BUSD vault Buffer equals 10%
            // And BUSD vault total BUSD deposit equals 1,000,000
            // When userA (anyone) call Earn at BUSD vault
            // Then Error "Not enough BUSD to be deployed to strategy"
        ...
        describe('Test Vault withdraw-1', () => {
            ...
            beforeEach(async (done) => {
                ...
                // prepare user Token balance(10000 BUSD) for testUser and deposit to vault
                await AccountUtils.giveERC20Token(strategyTokenSymbol, testUser, userTokenAmountToDeposit)
                await AccountUtils.doApprove(strategyTokenSymbol, testUser, vaultAddress, userTokenAmountToDeposit)
                await send(vault, 'deposit', [userTokenAmountToDeposit.toString()], { from: testUser })
                done()
            ...
            // Happy Path #1:
            // Given the bBUSD balance of userA’s wallet address(0x) equals 9900.9901
            // And bBUSD to BUSD exchange rate equals 1.0100
            // When userA withdraws 5000 bBUSD(returning 5000 bBUSD in exchange for 5050 BUSD)
            // And withdrawal fee 0.25% deduction equals 12.625 bBUSD
            // Then BUSD balance of userA’s wallet address(0x) equals 5,037.375
            // Then the bBUSD balance of userA’s wallet address(0x) equals 4900.9901
        ...
        describe('Test Vault withdraw-2', () => {
            ...
            // Happy Path #2:
            // Given the governance deposit 10000 BUSD to vault for 10000 bBUSD
            // And userA deposit 10000 BUSD to vault for 10000 bBUSD
            // And someone call earn 
            // And 5 day passed and some exchange happened in curve BusdPool
            // And withdrawal fee 0.25% deduction
            // And the governance withdraw all his bBUSD back based on bBUSD totalSupply and vault balance
            // When userA withdraws 10000 bBUSD
            // *and vault buffer is not enough to pay for withdrawal
            // Then userA will get BUSD based on bBUSD totalSupply and vault balance
        ...
        describe('Test Vault withdrawAll', () => {
            ...
            // Given the bBUSD balance of userA’s wallet address(0x) equals 9900.9901
            // And bBUSD to BUSD exchange rate equals 1.0100
            // When userA withdraws all BUSD(returning all bBUSD in exchange for BUSD)
            // Then the bBUSD balance of userA’s wallet address(0x) equals 0
            // And the BUSD balance of userA’s wallet address(0x) equals 10000
            // 0.25% withdrawal fee
        ...
        describe('Test Vault rebalance-1', () => {
            ...
            // Happy Path #1:
            // Given BUSD vault balance equals 80,000
            // And BUSD vault Buffer equals 10%
            // And BUSD vault total BUSD deposit equals 1,000,000
            // When userA (anyone) call Rebalance at BUSD vault
            // Then the amount of BUSD withdrawn from BUSD strategy back to BUSD vault equals 20,000
            // Then the remaining amount of BUSD in BUSD vault equals 100,000
        ...
        describe('Test Vault rebalance-2', () => {
            ...
            beforeEach(async (done) => {
                ...
                // call earn to invest to curve pool
                await send(vault, 'earn', [], { from: governance })
                // prepare user Token balance(10000 BUSD) for testUser and deposit to vault, to let vault buffer sufficient
                await AccountUtils.giveERC20Token(strategyTokenSymbol, testUser, userTokenAmountToDeposit)
                await AccountUtils.doApprove(strategyTokenSymbol, testUser, vaultAddress, userTokenAmountToDeposit)
                await send(vault, 'deposit', [userTokenAmountToDeposit.toString()], { from: testUser })
                done()
            ...
            // Unhappy Path #1:
            // Given BUSD vault balance equals 120,000
            // And BUSD vault Buffer equals 10%
            // And BUSD vault total BUSD deposit equals 1,000,000
            // When userA (anyone) call Rebalance at BUSD vault
