...
contract StrategyDaiV2 is CrvLocker {
    ...
    uint56 constant tokenIndexDai = uint56(TokenIndexInbusdPool.DAI);
    ...
    function deposit() public {
        ...
        /// dai -> bCrv Pool
        uint256[4] memory amounts = wrapCoinAmount(IERC20(want).balanceOf(address(this)), tokenIndexDai);
    ...
    function harvest(address gauge) public {
        ...
        uint256 bDaiBefore = IERC20(want).balanceOf(address(this));

        IUniswapRouter(unirouter).swapExactTokensForTokens(
            crvToWant, 1, swap2WantRouting, address(this), block.timestamp
        );

        uint256 bDaiAfter = IERC20(want).balanceOf(address(this));

        uint256 fee = bDaiAfter.sub(bDaiBefore).mul(manageFee).div(100);
    ...
    function _withdrawAll() internal {
        ...
        ICrvPoolZap(busdPoolZap).remove_liquidity_one_coin(amount, tokenIndexDai, 1);
    }
    ...
    function underlyingBalanceInPool() public view returns (uint256) {
        ...
        uint balanceUnderlying = ICrvPoolZap(busdPoolZap).calc_withdraw_one_coin(balance, tokenIndexDai);
