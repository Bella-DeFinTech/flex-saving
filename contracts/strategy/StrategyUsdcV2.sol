...
contract StrategyUsdcV2 is CrvLocker {
    ...
    uint256 constant public TO_BCRV_DECIMALS = 1e12; // 1e18 3bcrv / 1e6 usdc

    enum TokenIndexInbusdPool {DAI, USDC, USDT, BUSD}
    uint56 constant tokenIndexUsdc = uint56(TokenIndexInbusdPool.USDC);
    ...
    function deposit() public {
        ...
        /// usdc -> bCrv Pool
        uint256[4] memory amounts = wrapCoinAmount(IERC20(want).balanceOf(address(this)), tokenIndexUsdc);
    ...
    function harvest(address gauge) public {
        ...
        uint256 bUsdcBefore = IERC20(want).balanceOf(address(this));

        IUniswapRouter(unirouter).swapExactTokensForTokens(
            crvToWant, 1, swap2WantRouting, address(this), block.timestamp
        );

        uint256 bUsdcAfter = IERC20(want).balanceOf(address(this));

        uint256 fee = bUsdcAfter.sub(bUsdcBefore).mul(manageFee).div(100);
    ...
    function _withdrawAll() internal {
        ...
        ICrvPoolZap(busdPoolZap).remove_liquidity_one_coin(amount, tokenIndexUsdc, 1);
    }
    ...
    function balanceInPool() public view returns (uint256) {
        return ICrvDeposit(bCrvGauge).balanceOf(address(this))
            .mul(ICrvPoolUnderlying(busdPool).get_virtual_price()).div(1e18)
            .div(TO_BCRV_DECIMALS);
    }
    ...
    function underlyingBalanceInPool() public view returns (uint256) {
        ...
        uint balanceVirtual = balance.mul(ICrvPoolUnderlying(busdPool).get_virtual_price()).div(1e18).div(TO_BCRV_DECIMALS);
        uint balanceUnderlying = ICrvPoolZap(busdPoolZap).calc_withdraw_one_coin(balance, tokenIndexUsdc);
